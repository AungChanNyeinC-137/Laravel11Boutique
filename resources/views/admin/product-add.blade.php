...
<script>
   $(function() {
    // Image preview functionality for single image upload
    $("#myFile").on('change', function(e) {
        const [file] = this.files; // Correctly access the file
        if (file) {
            $('#imgpreview img').attr('src', URL.createObjectURL(file)); // Update image source
            $('#imgpreview').show(); // Show the preview container
        }
    });

    // Image preview functionality for gallery upload
    $("#gFile").on('change', function(e) {
        const gphotos = this.files; // Access the uploaded files
        $.each(gphotos, function(key, val) {
            $("#galUpload").prepend(
                `<div class="items gitems"><img src="${URL.createObjectURL(val)}" alt="gallery-image" /></div>`
            );
        });
    });
